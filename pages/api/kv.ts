...
async function get(
  ...
  res.send(variables.reduce<Record<string, any>>((acc, v) => {
    const [_, tag, key] = v.id.split(';')
    if (key) {
      acc[tag] = Object.assign(acc[tag] || {}, { [key]: JSON.parse(v.value) })
    } else {
...
async function patch(
  ...
  const flatBody = Object.entries(body).reduce<Record<string, any>>((acc, [tag, value]) => {
...
async function getUID(
  ...
  if (uid) {
    ...
    if (user) {
      ...
      const { valid, userId } = verifyToken(token as string, user.cookie_secret)
      if (!valid || userId !== uid) {
        res.status(400)
        throw new Error('User is not authenticated')
      }
