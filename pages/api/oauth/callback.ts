...
export default async function handler(
  ...
  try {
    ...
    const discord = await fetch(`https://discord.com/api/oauth2/token?grant_type=authorization_code&code=${code}&redirect_uri=${encodeURIComponent(process.env.DISCORD_REDIRECT ?? "")}`, {
      method: 'POST',
      headers: {
    ...
    let cookieSecret = user?.cookie_secret ?? crypto.randomBytes(16).toString('hex')
    if (!user) {
    ...
    setCookie('token', token.token, { req, res, domain: 'localhost', expires: new Date(token.expires * 1000) })
