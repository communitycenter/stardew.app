...
export const KVProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const api = useSWR('/api/kv', (...opts) => fetch(...opts).then(r => r.json()))
  ...
  const get = useCallback((tag: string, key?: string) => key ? kv[tag]?.[key] : kv[tag], [kv]);
  const set = useCallback(async (value: any, tag: string, key?: string) => {
    ...
    setKv(prev => Object.assign({}, prev, { [tag]: Object.assign({}, prev[tag], nv) }))
